...
trait TextSourceProcess {
  ...
  def source(path: String, charSet: Charset = StandardCharsets.UTF_8): Try[BufferedSource] = {
    ...
    if (trimmedPath.isEmpty)
      ...
      attempts.head
        .mapFailure(new TextSourceException(s"Failed to load text resource from '$trimmedPath' using the following attempts:\n" +
          traceLog, _))
    }
  }
}
