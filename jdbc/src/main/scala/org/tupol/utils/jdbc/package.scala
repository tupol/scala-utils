...
package object jdbc {
  ...
  implicit class ResultSetOps(rs: ResultSet) {
    ...
    def toIterator: Iterator[ResultSet]                                   =
  ...
  def query[T](sql: String)(implicit connection: Connection, extractor: RowExtractor[T]): Try[Iterable[T]] =
    Bracket
  ...
  def commitClose(implicit connection: Connection): Try[Unit] =
    Bracket.auto(connection)(_.commit())

  def rollbackClose(implicit connection: Connection): Try[Unit] =
