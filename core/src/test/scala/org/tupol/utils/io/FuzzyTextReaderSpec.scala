...
class FuzzyTextReaderSpec extends AnyFunSuite with Matchers {
  ...
  test("FuzzyTextReader.read successfully loads a text from a local path") {
    val path   = "core/src/test/resources/io/sample-text.resource"
  ...
  test("FuzzyTextReader.read successfully loads a text from the URI") {
    val path   = new java.io.File("core/src/test/resources/io/sample-text.resource").toURI.toASCIIString
    val result = read(path).get
    result shouldBe expectedText
  }

  test("FuzzyTextReader.read successfully loads a text from the URL") {
    val path   = new java.io.File("core/src/test/resources/io/sample-text.resource").toURI.toURL.toString
    val result = read(path).get
